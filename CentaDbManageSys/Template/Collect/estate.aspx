<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="estate.aspx.cs" Inherits="CentaDbManageSys.Template.Collect.estate" %>
  ...
  {%>     
	<li id="<%=item.EstateId %>" name="<%=item.EstateName %>">          
              <%if (item.Flow == FlowStatus.COLLECTING || item.Flow==FlowStatus.COLLECTING_ORDERSTATUS)
                { %> <div class="bg-yellow">收集中<%}
                else
                { %>
                <div class="bg-green">
                <%=item.ModifyDate.ToString("yyyy-MM-dd") %> 收集完成
                <%} %>
        </div>        			
